...
const GLchar* vertexSource =
	...
	"	view_direction = normalize(view_position - Position);"
	"	Coord = coord;"
...
const GLchar* fragmentSource =
	...
	"void main() {"
	"	vec3 R = reflect(-light_direction, Normal);"
	"	vec3 R1 = reflect(-light1_direction, Normal);"
	"	vec3 Color = light_intensity*material_ambient;"
	"	Color += (light1_intensity*material_diffuse * max(dot(light1_direction, Normal), 0.0))+(light_intensity*material_diffuse * max(dot(light_direction, Normal), 0.0));"
	"	Color += (light1_intensity*material_specular * pow(max(dot(R1, view_direction), 0.0), shininess))+(light_intensity*material_specular * pow(max(dot(R, view_direction), 0.0), shininess));"
	"	outColor = mix(vec4(Color, 1.0),texture(ambientSampler, Coord),0.5) ;"
	"}";
...
void draw(GLFWwindow* window)
{
	...
	view=glm::rotate(view,3.14f,glm::vec3(0.0f,0.0f,1.0f));
	glm::mat4 model(1.f);
	...
	glm::vec3 light1_direction = glm::normalize(glm::vec3(-2.f, -2.f, -3.f));
	glm::vec3 light1_intensity = glm::vec3(1.f, 1.f, 0.f);
	...
	glUniform3fv(glGetUniformLocation(shaderProgram, "light1_direction"), 1, &light1_direction[0]);
	glUniform3fv(glGetUniformLocation(shaderProgram, "light1_intensity"), 1, &light1_intensity[0]);
