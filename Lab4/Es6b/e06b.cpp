...
/*"in vec3 Position;"
	...
	"	outColor = vec4(material_ambient, 1.0);"
	"	outColor += vec4(material_diffuse, 1.0) * texture(textureSampler, Coord) * max(dot(light_direction, Normal), 0.0);"
...
void initialize_texture()
{
	...
	error = lodepng::decode(image2, width, height, "../assets/sphere/" + material[0].specular_texname);
	if (error) std::cout << "decode error " << error << ": " << lodepng_error_text(error) << std::endl;
...
void draw(GLFWwindow* window)
{
	...
	glUniform1f(glGetUniformLocation(shaderProgram, "Opacita_texture"), 0.5f);
