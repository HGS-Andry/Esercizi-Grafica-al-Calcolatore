...
glm::vec3 camera_position(0.0f, 5.0f, 6.0f);
glm::vec3 camera_direction(0.0f, -0.5f, -1.0f);
...
const GLchar* fragmentSource =
	...
	"const vec4 fogColor = vec4(0.0, 0.0,0.0,1);"
	"const float FogDensity = 0.05;"
	"void main() {"
	"	vec3 light_dir_calcolata=normalize(light_direction - Position);"
	"	vec3 view_direction = normalize(view_position - Position);"
	"	vec3 R = normalize(reflect(-light_dir_calcolata, Normal));"
	"	outColor = vec4(light_intensity*material_ambient*texture(textureSampler, Coord), 1.0);"
	"	outColor += vec4(0*light_intensity*material_diffuse*texture(textureSampler, Coord), 1.0) * max(dot(light_dir_calcolata, Normal), 0.0) ;"
	"	outColor += vec4(light_intensity*material_specular*500, 1.0) * pow(max(dot(R, view_direction), 0.0), shininess);"


	/*"	vec3 Color = light_intensity*material_ambient*texture(texture_Diffuse, Coord);"
	"	Color += light_intensity*texture(texture_Diffuse, Coord) * max(dot(light_direction, Normal), 0.0);"
	"	Color += light_intensity*material_specular*texture(texture_specular, Coord) * pow(max(dot(R, view_direction), 0.0), shininess);"
	"	outColor = vec4(Color, 1.0);"
	max(dot(light_dir_calcolata, Normal), 0.0)*/
	"}";
...
const GLchar* fragmentSource2 =
	...
	"uniform vec3 view_position;"	
	"void main() {"
	"	outColor = vec4(mix(material_diffuse,material_specular,fattore), 1);"
	"}";
...
void draw(GLFWwindow* window)
{
	...
	glm::vec3 light_direction = glm::vec3(0.f, 0.f, 0.f);
	glm::vec3 colorerosso =glm::vec3(1.f, 0.f, 0.f);
	glm::vec3 coloregiallo = glm::vec3(1.f, 0.847058824f, 0.f);
	...
	glUniform3fv(glGetUniformLocation(shaderProgram2, "material_diffuse"), 1, &coloregiallo[0]);
	glUniform3fv(glGetUniformLocation(shaderProgram2, "material_specular"), 1,  &colorerosso[0]);
	glUniform1f(glGetUniformLocation(shaderProgram2, "fattore"), sin(t));
	...
	model *= glm::translate(glm::mat4(1.f), glm::vec3(2*sin(1), 0.f, 2*cos(1)));
	model *= glm::scale(glm::mat4(1.f), glm::vec3(0.3333f, 0.3333f, 0.3333f));
	...
	model = glm::translate(glm::mat4(1.f), glm::vec3(3*sin(2*t), 0.f, 3 * cos(2*t)));
	model *= glm::scale(glm::mat4(1.f), glm::vec3(0.15f, 0.15f, 0.15f));
	glUniformMatrix4fv(glGetUniformLocation(shaderProgram2, "projection"), 1, GL_FALSE, &projection[0][0]);
	glUniformMatrix4fv(glGetUniformLocation(shaderProgram2, "view"), 1, GL_FALSE, &view[0][0]);
	glUniformMatrix4fv(glGetUniformLocation(shaderProgram2, "model"), 1, GL_FALSE, &model[0][0]);
	glUniform3fv(glGetUniformLocation(shaderProgram2, "light_direction"), 1, &light_direction[0]);
	...
	glUniform3fv(glGetUniformLocation(shaderProgram2, "material_specular"), 1, &colorerosso[0]);
	glUniform1f(glGetUniformLocation(shaderProgram2, "fattore"), 1);
	glBindVertexArray(vao[1]);
	glDrawElements(GL_TRIANGLES, element_count[1], GL_UNSIGNED_INT, 0);
