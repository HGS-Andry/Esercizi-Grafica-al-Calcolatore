...
void draw(GLFWwindow* window)
{
	...
	glm::mat4 projection = glm::perspective(PI/4, 1.f/1.f, 0.1f, 10.0f);
	...
	glm::mat4 model = glm::rotate(glm::mat4(1.f), float(val/1500.f), glm::vec3(0.0f, 1.0f, 0.0f)); //Per ruotarlo
...
int main(int argc, char const *argv[])
{
	...
	while(!glfwWindowShouldClose(window))
	{
		...
		glfwPollEvents();
		if (val == 539999)//solo per animazione
			val = 0;
		val++;
	}
